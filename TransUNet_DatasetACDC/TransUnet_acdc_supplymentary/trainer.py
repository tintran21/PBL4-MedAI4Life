    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num >= max_iterations:
                break
        # --- THÊM VÀO TỪ ĐÂY (Lưu ý lề ngang bằng với chữ "for i_batch") ---
        # 1. Lưu checkpoint mỗi 7 epoch
        if (epoch_num + 1) % 7 == 0:
            save_interval_path = os.path.join(snapshot_path, 'epoch_' + str(epoch_num) + '.pth')
            torch.save(model.state_dict(), save_interval_path)
            logging.info("=> Đã lưu checkpoint định kỳ tại: {}".format(save_interval_path))

            # 2. Luôn lưu model của epoch gần nhất để phòng hờ rớt mạng
